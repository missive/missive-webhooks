...
class GitHub extends Integration {
  ...
  fork (payload) {
    ...
    var text = this.sanitize(() => `${repository.name} was forked by ${sender.login} at ${this.link(forkee.html_url, forkee.full_name)}`)
  ...
  issues(payload) {
    ...
    var title, title_link, text, markdown, pretext = this.sanitize(() => `${this.link(issue.html_url, 'Issue')} ${payload.action} by ${sender.login}`)
