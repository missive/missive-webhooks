  ...
  pull_request(payload) {
    ...
    if (action == 'assigned' || action == 'unassigned') {
      ...
      if (action == 'review_requested') {
        ...
        theAction = 'removed the request review'
      }

      pretext = this.sanitize(() => `${sender.login} ${theAction} from ${requested_reviewer.login} on ${this.pullRequestLink(pull_requestm repository)}`)
    } else {
  ...
  push (payload) {
    ...
    if (includeCommits && size > 0) {
      ...
      commits.forEach((commit) => {
        ...
        var line = `${this.codeLink(this.commitUrl(commit), this.shortCommit(commit.id))} ${msg}`
