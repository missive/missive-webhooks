...
class GitHub extends Integration {
  ...
  commit_comment (payload) {
    ...
    return {
      references: [`${repository.id}/${comment.commit_id}`],
      subject: `[${repository.name}] ${this.shortCommit(comment.commit_id)}`,
  ...
  create (payload) {
    ...
    return {
      references: [`${repository.id}/${ref}`],
      subject: `[${repository.name}] ${ref}`,
  ...
  issue_comment(payload) {
    ...
    return {
      references: [this.referenceIdForIssue(issue, repository)],
  ...
  pull_request(payload) {
    ...
    return {
      references: [this.referenceIdForIssue(pull_request, repository)],
  ...
  pull_request_review(payload) {
    ...
    var pretext = this.sanitize(() => `New ${this.link(review.html_url, 'review')} by ${this.userLink(sender)}`)
  ...
  push (payload) {
    ...
    return {
      ...
      subject: `[${repository.name}] ${branch}`,
      notification: notification,
  ...
  subjectForIssue(issue, repository) {
    return `[${repository.name}] #${issue.number} ${issue.title}`
  }
}
