...
class GitHub extends Integration {
  ...
  push (payload) {
    ...
    var text = [], pretext, notification, branch, size, includeCommits, color

    if (!ref.startsWith('refs/heads/')) return

    branch = ref.replace('refs/heads/', '')
    if (branch.startsWith('gh-readonly-queue')) return
    ...
    return {
      ...
      callback: (conversationId, { createPost }) => {
        ...
        for (const { conversation, commit } of referencedConversations) {
          ...
          let text = this.sanitize(() => `${commit.author.username} referenced this conversation in ${this.codeLink(this.commitUrl(commit), this.repositoryCommit(repository, commit))}`)
  ...
  repositoryCommit(repository, commit) {
    return `${repository.full_name}@${this.shortCommit(commit.id)}`
  }
