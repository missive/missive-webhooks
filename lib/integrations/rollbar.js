...
class Rollbar extends Integration {
  ...
  linksForItem(item, occurence) {
    ...
    if (occurence) {
      let occurenceURL = `${itemURL}/occurrences/${last_occurrence_id}`
      return `${this.link(itemURL)} (${this.link(occurenceURL, 'Occurence')})`
    } else {
