...
const EVENT_CALLBACKS = {
  ...
  'order.payment_succeeded': 'order_payment_succeeded',
...
class Stripe extends Integration {
  ...
  order_created (payload) {
    var { customer, amount, currency, status } = payload
    var converted_amount = this.convert_currency(amount, currency)
    var text = this.sanitize(() => `An order has been created`)
    ...
    return {
      ...
      attachment: {
        pretext: text.toString(),
        markdown: `_Amount_: **${converted_amount}**, _Status_: **${status}**`
      }
    }
  }

  order_payment_failed (payload) {
    var { customer, amount, currency, status } = payload
    var converted_amount = this.convert_currency(amount, currency)
    var text = this.sanitize(() => `An order payment attempt failed`)

    return {
      references: [customer],
      subject: 'Order paymend failed',
      notification: text.sanitized,
      attachment: {
        pretext: text.toString(),
        markdown: `_Amount_: **${converted_amount}**, _Status_: **${status}**`
  ...
  order_payment_succeeded (payload) {
    var { customer, amount, currency, status } = payload
    var converted_amount = this.convert_currency(amount, currency)
    var text = this.sanitize(() => `An order payment attempt succeeded`)

    return {
      references: [customer],
      subject: 'Order paymend succeeded',
      notification: text.sanitized,
      attachment: {
        pretext: text.toString(),
        markdown: `_Amount_: **${converted_amount}**, _Status_: **${status}**`
